...
<div class="mt-6 mb-6 ml-2 flex justify-center">
	<nav
		class="flex items-center space-x-1 overflow-x-auto rounded-xl bg-gray-500/5 p-1 text-sm text-gray-600 dark:bg-gray-500/20"
	>
		<button
			role="tab"
			type="button"
			...
			class:text-black={activeTab === 'forms'}
			class:shadow={activeTab === 'forms'}
			class:text-gray-400={activeTab !== 'forms'}
		...
		<button
			...
			class:text-black={activeTab === 'shared'}
			class:shadow={activeTab === 'shared'}
			class:text-gray-400={activeTab !== 'shared'}
...
{#if activeTab === 'forms'}
	...
	{/if}
{:else}
	<p class="text-center text-gray-600">No shared forms found.</p>
{/if}
